...
scores_novdec %>% 
  ggplot(aes(x = target_end_date, y = wis, color = model)) + geom_line() + labs(title = "WIS Score by Model", x = "Date", y = "WIS Score", color = "Location")
```

##Fig3: Line graph faceted by location 
## Note: Need to add in dotted lines for state specific policy changes
```{r}
scores_novdec_location <- scored_forecasts_novdec %>%
  dplyr::group_by(location, model, target_end_date) %>% 
  dplyr::summarise(wis = mean(wis)) 
scores_novdec_location %>% 
  ggplot(aes(x = target_end_date, y = wis, color = model)) + geom_line() + facet_wrap(.~location) + labs(title = "WIS Scores by State and Model", x = "Date", y = "WIS Score", color = "Location")
```
